...
awaitable<void> UPackageCodecImpl::Encode(ATcpSocket &socket, IPackage *pkg) {
    try {
        ...
        if (const auto len = co_await async_write(socket, asio::buffer(&tmp->header_, FPackage::headerSize)); len == 0) {
        ...
        co_await async_write(socket, asio::buffer(tmp->data_));
...
awaitable<void> UPackageCodecImpl::Decode(ATcpSocket &socket, IPackage *pkg) {
    try {
        ...
        if (const auto len = co_await async_read(socket, asio::buffer(&tmp->header_, FPackage::headerSize)); len == 0) {
        ...
        co_await async_read(socket, asio::buffer(tmp->data_));
    } catch (std::exception &e) {
        spdlog::error("{} - {}", __func__, e.what());
    }
    co_return;
