 ...
 * Script version: v%s
 * Source file: /sql/%s
 */\n\n''' % (date_str, platform.python_version(), VERSION, file_name))
        ...
        for table in table_list:
            ...
            for field in table['field'].values():
                ...
                if 'int' in code_type_map[field['type']]:
                    construct_str = construct_str + code_type_map[field['type']] + ' ' + field['name'] + ", \n\t\t\t"
                else:
                    construct_str = construct_str + code_type_map[field['type']] + ' ' + field['name'] + ", "
...
for file_name, table_list in sql_list.items():
    with open(os.path.join(GENERATED_DIR, file_name + '.orm.cpp'), 'w', encoding='utf-8') as file:
 ...
 * Python version: v%s
 * Script version: v%s
 */\n\n''' % (date_str, platform.python_version(), VERSION))
        ...
        for table in table_list:
            ...
            # Query
            file.write('\tvoid DBTable_%s::Query(mysqlx::Schema &schema) {\n' % to_upper_camel_case(table['name']))
